...
// חסימה / ביטול חסימה קבועה של משתמש מגישה לצ'אט-בוט - צוות (מנהל/פיקוח) בלבד.
// המשתמש עדיין יכול להשתמש בשאר האתר, כולל פתיחת פנייה לתמיכה - זו דרך הערר.
export async function POST(request: Request) {
  ...
  const { error } = await admin
    ...
    .update({
      bot_banned: banned,
      bot_ban_reason: banned ? (reason || "").toString().slice(0, 300) || null : null,
      // פעולת צוות מפורשת (חסימה או ביטול) מבטלת גם כל חסימת-שעה אוטומטית שעדיין בתוקף.
  ...
  await logAudit({
    actorId: user.id,
    action: banned ? "bot_ban_user" : "bot_unban_user",
    targetType: "user",
    targetId: userId,
