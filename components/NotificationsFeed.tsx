...
import { Bell, Loader2, Package, Sparkles, Lightbulb, Users, ShieldAlert, KeyRound, AlertTriangle, ServerCrash } from "lucide-react";
...
function iconFor(kind: string) {
  ...
  if (kind === "bot_notice") return <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-gold" />;
  if (kind === "bot_down") return <ServerCrash className="mt-0.5 h-4 w-4 shrink-0 text-red-400" />;
  if (kind === "password_reset") return <KeyRound className="mt-0.5 h-4 w-4 shrink-0 text-gold" />;
